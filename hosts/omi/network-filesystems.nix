...

{
  ...
  fileSystems."/mnt/nextcloud_backup" = {
    ...
    options = [	
      ...
      "rw" "username=${secrets.hetzner.username}" "password=${secrets.hetzner.password}"
      "uid=33" "gid=0" "file_mode=0660" "dir_mode=0770"
    ];
  };
  ...
  fileSystems."/mnt/immich_backup" = {
    ...
    options = [ 
      ...
      "x-systemd.mount-timeout=5s"
      "rw" "username=${secrets.hetzner.username}" "password=${secrets.hetzner.password}"
      "uid=1000" "gid=100" "file_mode=0660" "dir_mode=0770"
    ];
  };
}
